...
<!-- Encabezado -->
    ...
    <script>
        // URL de la API
        const apiUrl = 'http://localhost:8080/WebApplication1/data3.jsp';
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    const maxProductionCell = document.createElement('td');
                    maxProductionCell.textContent = item.maxProduction.toFixed(2);
                    row.appendChild(maxProductionCell);

                    const minProductionCell = document.createElement('td');
                    minProductionCell.textContent = item.minProduction.toFixed(2);
                    row.appendChild(minProductionCell);

                    const avgProductionCell = document.createElement('td');
                    avgProductionCell.textContent = item.avgProduction.toFixed(2);
                    row.appendChild(avgProductionCell);

                    const maxConsumptionCell = document.createElement('td');
                    maxConsumptionCell.textContent = item.maxConsumption.toFixed(2);
                    row.appendChild(maxConsumptionCell);

                    const minConsumptionCell = document.createElement('td');
                    minConsumptionCell.textContent = item.minConsumption.toFixed(2);
                    row.appendChild(minConsumptionCell);

                    const avgConsumptionCell = document.createElement('td');
                    avgConsumptionCell.textContent = item.avgConsumption.toFixed(2);
...
<script>
    ...
    fetch("http://localhost:8080/WebApplication1/data3.jsp")
        .then(response => response.json())
        .then(data => {
            // Procesar los datos para el gráfico
            const labels = data.map(item => item.name); // Nombres de las fuentes de energía
            const maxProduction = data.map(item => item.maxProduction);
            const avgProduction = data.map(item => item.avgProduction);
