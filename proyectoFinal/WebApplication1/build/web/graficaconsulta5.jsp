...
<!-- Encabezado -->
    ...
    <script>
        // URL de la API
        const apiUrl = 'http://localhost:8080/WebApplication1/data5.jsp';
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    yearCell.textContent = item.year.toFixed(0);
                    row.appendChild(yearCell);

                    const averageCell = document.createElement('td');
                    averageCell.textContent = item.averageConsumption.toFixed(2);
...
<script>
    ...
    fetch("http://localhost:8080/WebApplication1/data5.jsp")
        .then(response => response.json())
        .then(data => {
            // Procesar los datos para el gráfico
            const labels = data.map(item => item.year); // Nombres de las fuentes de energía
            const Average = data.map(item => item.averageConsumption);
