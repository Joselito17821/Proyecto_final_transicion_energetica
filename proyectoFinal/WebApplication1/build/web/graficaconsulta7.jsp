...
<!-- Encabezado -->
    ...
    <table id="dataTable" class="display">
        <thead>
            <tr>
                ...
                <th>Diferencia de Produccion</th>
    ...
    <script>
        // URL de la API
        const apiUrl = 'http://localhost:8080/WebApplication1/data7.jsp';
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    const productionDifferenceCell = document.createElement('td');
                    productionDifferenceCell.textContent = item.productionDifference.toFixed(2);
                    row.appendChild(productionDifferenceCell);
...
<script>
    ...
    fetch("http://localhost:8080/WebApplication1/data7.jsp")
        .then(response => response.json())
        .then(data => {
            // Procesar los datos para el gráfico
            const labels = data.map(item => item.name); // Nombres de las fuentes de energía
            const Difference = data.map(item => item.productionDifference);
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: 'Diferencia de Producción',
                            data: Difference,
